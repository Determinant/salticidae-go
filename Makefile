...
build/test_msgnet: build/libsalticidae.a test_msgnet/main.go
	bash -c 'source scripts/env.sh && go build -o $@ github.com/ava-labs/salticidae-go/test_msgnet'
build/test_msgnet_tls: build/libsalticidae.a test_msgnet_tls/main.go
	bash -c 'source scripts/env.sh && go build -o $@ github.com/ava-labs/salticidae-go/test_msgnet_tls'
build/test_p2p_stress: build/libsalticidae.a test_p2p_stress/main.go
	bash -c 'source scripts/env.sh && go build -o $@ github.com/ava-labs/salticidae-go/test_p2p_stress'
build/bench_network: build/libsalticidae.a bench_network/main.go
	bash -c 'source scripts/env.sh && go build -o $@ github.com/ava-labs/salticidae-go/bench_network'
